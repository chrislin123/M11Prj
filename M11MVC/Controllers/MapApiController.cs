...
namespace M11MVC.Controllers
{
    ...
    public class MapApiController : BaseApiController
    {
        ...
        public List<dynamic> getCoordinate()
        {
            ...
            var Params = lib.M11MVCLib.ParseQueryString(ActionContext.Request.RequestUri.Query);
            ...
            string ssql = @" 
                ...
                where 1=1 {0}
                order by c.villageid,c.Pointseq
            ";
            if (sType == "R")
            {
                ssql = string.Format(ssql, " and a.status = 'A3' ");
            }

            if (sType == "Y")
            {
                ssql = string.Format(ssql, " and a.status in ('A1','A2') ");
            }
        ...
        public List<dynamic> getMapDatas()
        {
            ...
            if (sType == "R")
            {
                ssql += " and a.status = 'A3' ";
            }

            if (sType == "Y")
            {
                ssql += " and a.status in ('A1','A2') ";
            }
